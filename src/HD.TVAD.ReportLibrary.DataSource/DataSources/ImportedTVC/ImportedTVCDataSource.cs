...
{
	public class ImportedTVCDataSource : DataSource<ImportedTVCViewModel>
	{

		public ImportedTVCDataSource(IServiceProvider serviceProvider, ImportedTVCParameter parameter)
		{
			...
			foreach (var spot in spots)
			{
				...
				{
					Data.Add(new ImportedTVCViewModel()
					{
			...
			{
				ImportedTVCViewModel rBegin = Data[i];
				int j = i + 1;
				for (; j < Data.Count; j++)
				{
					ImportedTVCViewModel rIndex = Data[j];
					if (rIndex.BroadcastDate != rBegin.BroadcastDate ||
						rIndex.ChannelName.ToUpper() != rBegin.ChannelName.ToUpper() ||
						rIndex.Page != rBegin.Page)
						break;
				}
				TachGhiChu((List<ImportedTVCViewModel>)Data, i, j);
				i = j;
			}

		}
		private void TachGhiChu(List<ImportedTVCViewModel> dtLichPS, int rowbegin, int rowend)
		{
			string GhiChuKenh = "";
