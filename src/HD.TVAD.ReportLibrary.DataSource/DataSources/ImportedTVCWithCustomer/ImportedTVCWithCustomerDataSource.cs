...
{
	public class ImportedTVCWithCustomerDataSource : DataSource<ImportedTVCWithCustomerViewModel>
	{

		public ImportedTVCWithCustomerDataSource(IServiceProvider serviceProvider, ImportedTVCWithCustomerParameter parameter)
		{
			...
			foreach (var spot in spots)
			{
				...
				{
					Data.Add(new ImportedTVCWithCustomerViewModel()
					{
						...
						Page = 0,
						CutOrder = 0,
						OnAirIndex = spotBooking.IsApproved ? spotBooking.SpotAssetByBookings.SpotAsset.Index
							: spotBooking.Position.GetValueOrDefault(),
			...
			Data = Data.OrderBy(a => a.BroadcastDate)
				...
				.ThenBy(a => a.Page)
				.ThenBy(a => a.CutOrder)
				.ThenBy(a => DataSource.Util.SplitLetter(a.TimeBandName))
				.ThenBy(a => DataSource.Util.SplitNumber(a.TimeBandName))
				.ThenBy(a => a.TimeBandName)
				.ThenBy(a => DataSource.Util.ToInt(a.TimeBandSlice))
			...
			{
				ImportedTVCWithCustomerViewModel rBegin = Data[i];
				int j = i + 1;
				for (; j < Data.Count; j++)
				{
					ImportedTVCWithCustomerViewModel rIndex = Data[j];
					if (rIndex.BroadcastDate != rBegin.BroadcastDate ||
						rIndex.ChannelName.ToUpper() != rBegin.ChannelName.ToUpper() ||
						rIndex.Page != rBegin.Page)
						break;
				}
				TachGhiChu((List<ImportedTVCWithCustomerViewModel>)Data, i, j);
				i = j;
			}

		}
		private void TachGhiChu(List<ImportedTVCWithCustomerViewModel> dtLichPS, int rowbegin, int rowend)
		{
			string GhiChuKenh = "";
