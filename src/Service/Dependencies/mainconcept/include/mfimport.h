...
/*!
 ...
 @details The path must be a null terminated string, must contain a trailing slash
  ...
  To change the path for a demuxer, set demuxer_type to one of the following:
    ...
    mcmjtMXF            : changes the mxf demuxer path
    mcmjtMP4            : changes the mp4 demuxer path
    mcmjtAVI,
    mcmjtMKV            : changes the dmf demuxer path
    mcmjtASF            : changes the asf demuxer path

  To change the path of a decoder, set demuxer_type = mcmjtElementary and set
  decoder_type to one of the following:

    mctMPEG2V     : changes the mpeg-1/2 video decoder path
    mctMPEG4V     : changes the mpeg-4 video decoder path
    mctH264       : changes the h264 video decoder path
    mctHEVC       : changes the hevc video decoder path
    mctVC1        : changes the vc1 video decoder path
    mctDV         : changes the dv video decoder path
    mctJ2K        : changes the j2k video decoder path
    mctMotionJPEG : changes the mjpeg video decoder path
    mctDIV4       : changes the divx video decoder path
    mctMVC        : changes the mvc video decoder path
    mctVC3        : changes the vc3 video decoder path

    mctMPEG2A     : changes the mpeg audio decoder path
    mctAAC_LATM,
    mctAAC_ADTS   : changes the aac audio decoder path
    mctAC3        : changes the dolby audio decoder path
    mctAMR        : changes the amr audio decoder path
    mctWMA        : changes the wma audio decoder path
    mctDTS        : changes the dts audio decoder path
...
/*!
 ...
 @details See mfimportSetComponentPath
...
/*!
@brief Frame info for the externalGetFrameInfo callback.
...
the get_rc system will be called with the string 'extGetFrameInfo'. To enable external get frame info mode,
supply a function pointer of this type to the get_rc call like:

int32_t external_get_frame_info(void* appData, int64_t storageFrame, externalFrameInfo *pInfo)
{
...
}
...
{
    if (!strcmp(name, "extGetFrameInfo"))
        return (void*) external_get_frame_info;
...
}
...
struct mpegInInfo
{
    ...
    mpegInProg inProg;                 //!< deprecated, use get_rc prg_printf instead
    ...
    int32_t YUVFormat;                 //!< deprecated, use the use_external_frame_buffer and frame_info fields instead
    ...
    //! @name deprecated input items
    //!@{
    int32_t (*prompt_index_mode)();  //!< deprecated, pass a get_rc function pointer to mpegInOpen instead
    ...
    int32_t reserved11[5892];        //!< reserved for future use
    ...
    //! @name deprecated input items
    //!@{
    void  (*err_printf)(const char *fmt, ...);                  //!< deprecated, pass a get_rc function pointer to mpegInOpen instead
    void  (*prg_printf)(int32_t percent, const char *fmt, ...); //!< deprecated, pass a get_rc function pointer to mpegInOpen instead
    void  (*wrn_printf)(const char *fmt, ...);                  //!< deprecated, pass a get_rc function pointer to mpegInOpen instead
    void  (*inf_printf)(const char *fmt, ...);                  //!< deprecated, pass a get_rc function pointer to mpegInOpen instead
    void *(*memAlloc)(uint32_t size);                           //!< deprecated, pass a get_rc function pointer to mpegInOpen instead
    void  (*memFree)(void *memblk);                             //!< deprecated, pass a get_rc function pointer to mpegInOpen instead
    void *(MC_EXPORT_API *get_rc)(const char* name);            //!< deprecated, pass a get_rc function pointer to mpegInOpen instead
...
/*!
 @brief Call to save the index data to disk files, Unicode version.
 @details See mpegInSetComponentPath
