...
#define COMP_SONY_XAVCS     19/*!< \brief Sony XAVC File Format
                              preset settings:
                              - KeepAUDelimiter:    on
                              - KeepPS:             off
                              - RemoveFiller:       off
                              ...
                              FTYPE:
                              - BrandName:          BrandNameXAVC
                              - MinorVersion:       0
                              - Compatibilities:    BrandNameXAVC, BrandNameTypeMP42, BrandNameTypeISOM
                              */
...
/*! \brief Get mux time
*/
#define GET_MUX_TIME        0x00000005L
...
struct mp4_m_settings
{
    ...
    char *output_path;                          //!< UTF-8 encoded string that represents folder where temporary file will be created
    int capture_mode;                           //!< TODO:  make documentation
    ...
    int segmentation_mode;                      //!< segmentation mode: <br>
                                                //!< 0 - general mode (segment samples by timestamps only); <br>
                                                //!< 1 - custom mode (muxer will wait for sync AAC frame to separate segments). 
                                                //!< Used for DASH segmentation with HE and HEv2; 
                                                //!< muxer needs to obtain \"auxinfo\" message with @ref AAC_SYNC_FRAME_INFO identifier
    uint64_t FirstFragmentTimeOffset;            //!< specifies an absolute decode time for the first sample in the first fragment in 100NanoSecond units. 
                                                 //!< Can be applied only for DASH and UVU compatibility modes.
    DECLARE_RESERVED_FIELD(981*4, 4);           //!< Reserved for future use
    // total 4096 bytes
};
