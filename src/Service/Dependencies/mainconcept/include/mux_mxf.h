...
//! @{
#define MXF_PROF_SONY_XDCAM         0     //!< Sony XDCAM and XAVC type files (OP1a frame-wrapped essences)  
#define MXF_PROF_PANASONIC_P2       3     //!< Panasonic P2 type files (Op1b frame-wrapped or OpAtom clip-wrapped essence)
#define MXF_PROF_DCI_2K             6     //!< DCI 2K MXF files (OpAtom frame-wrapped essences, each stream in a separate file)
#define MXF_PROF_DCI_4K             7     //!< DCI 4K MXF files (OpAtom frame-wrapped essences, each stream in a separate file)
...
//! @{
#define MXF_MUX_TYPE_XDCAM_HD       32		/*!< @brief Sony XDCAM-HD compliant MPEG-2 video and AES-3 382M audio; OP1a SMPTE 378M. (SMPTE RDD-9)*/
#define MXF_MUX_TYPE_XDCAM_IMX      28		/*!< @brief Sony XDCAM IMX compliant MPEG D-10 video and AES-3 331M audio; OP1a SMPTE 378M. */
#define MXF_MUX_TYPE_XDCAM_DV       99		/*!< @brief Sony XDCAM DV compliant DV-DIF video and AES-3 382M audio; OP1a SMPTE 378M. */

#define MXF_MUX_TYPE_XDCAM_SXS_HD   50		/*!< @brief Sony Professional Memory Card (SxS) XDCAM HD File Format */
#define MXF_MUX_TYPE_XDCAM_SXS_IMX  51		/*!< @brief Sony Professional Memory Card (SxS) XDCAM IMX File Format */
#define MXF_MUX_TYPE_XDCAM_SXS_DV   52		/*!< @brief Sony Professional Memory Card (SxS) XDCAM DV File Format */

#define MXF_MUX_TYPE_XAVC_SXS       53		/*!< @brief Sony XAVC File Format, Single Essence Location Style. (SMPTE RDD-32 7.3.1)
                                               XAVC compliant AVC-Intra CBR, VBR and AVC-LongGOP video and up to 16 channels of AES-3 382M audio.  */

#define MXF_MUX_TYPE_XAVC_MEL       54		/*!< @brief Sony XAVC File Format, Multiple Essence Location Style. (SMPTE RDD-32 7.3.2)
                                               XAVC compliant AVC-Intra VBR and AVC-LongGOP video and up to 16 channels of AES-3 382M audio.  */
                                               /*!< @details Do not use for XAVC-Intra CBR.*/

#define MXF_MUX_TYPE_P2_DVCPRO      100		/*!< @brief Panasonic P2 compliant DV/DVCPRO video and AES-3 382M audio; Specialized Operational Pattern Atom SMPTE 390M. */
#define MXF_MUX_TYPE_P2_AVCI        101		/*!< @brief Panasonic P2 compliant AVC-Intra 50/100/200 video and AES-3 382M audio; Specialized Operational Pattern Atom SMPTE 390M. */
#define MXF_MUX_TYPE_P2_AVC_LG      102		/*!< @brief Deprecated, use MXF_MUX_TYPE_P2_AVC_ULTRA */
#define MXF_MUX_TYPE_P2_AVC_ULTRA   102		/*!< @brief Panasonic P2 AVC-Ultra compliant AVC-Intra or AVC-LongGOP video and AES-3 382M audio; Op1b SMPTE 391M */
...
#define MXF_MUX_FLAG_DROP_FRAME       0x00000001	//!< @brief Use drop frame timecode.
                                                                            /*!< @details If the DV essence had timecode embedded
...
#define MXF_MUX_FLAG_XDCAM_11_COMP    0x00000002	//!< @brief force old XDCAM 1.1 spec. format.  
                                                                              /*!< @details Only use if the output streams compatibility with old applications that do not
                                                                                ...
                                                                                //!< @details Used for calculating EditUnitByteCount in Index Table 
...
#define MXF_MUX_FLAG_AVC_ST381_3		0x00000200	//!< @brief Flag to specify descriptor style for AVC in generic profiles.
                                                /*!< @details Use CDCIDescriptor/AVCSubdescriptor according to SMPTE 381-3
                                                      ...
                                                      However do not set these flags if the stream parameters are not clearly known.
...
#define MXF_MUX_FLAG_AVC_INTRA_4K_CBG  0x00001000 //!< @brief Flag to indicate that the XAVC Intra 4K video stream has constant bitrate.
                                                      /*!< @details Used to decide index table style.
                                                        ...
                                                        Must be Used in combination with XAVC Intra class flags below */
...
#define MXF_MUX_FLAG_INCR_INDEX_WRITE   0x00010000 //!< @brief Flag to specify incremental index file writing mode
                                                   //!< @details Unless set - write whole index every time when mxfMuxWriteIndexIntoFile() is called
...
#define MXF_METADATA_DMS_P2_XML       4  //!< P2 AVC-Ultra Op1b descriptive metadata, XML meta data 
#define MXF_METADATA_DMS_P2_BMP       5  //!< P2 AVC-Ultra Op1b descriptive metadata, BMP Thumbnail 
//! @}

//! @name Descriptive Metadata track types
...
//! @name input stream flags
//! @brief Used with \ref  mxf_stream_set_struct.stream_flags field
//! @{
#define MXF_STREAM_FLAG_SEPARTE_FIELDS 0x00000001  //!< @brief Set when a sequence of JPEG2000 samples consists of encoded source video fields rather than frames.
#define MXF_STREAM_FLAG_BFF            0x00000002  //!< @brief Set when a sequence of JPEG2000 encoded fields has field order bottom field first (default top field first).
                                                   //! @details Note: Edit rate for JPEG2000 encoded fields is the field rate. Set mxf_m_settings.video_framerate to field rate.

#define MXF_STREAM_FLAG_VBE            0x00000004  //!< @brief Set when input ST 436 ANC or VBI Frame elements are of variable size (VBE). Default is constant size (CBE)
//! @}
...
//! @brief Used to specify Panasonic P2 AVC-Ultra Op1b Descriptive Metadata in \ref mxf_dm_meta_data
...
{
    int32_t dm_type;                  //!< @brief Descriptive Meta Data type, one of MXF_METADATA_DMS_xx
...
//! @brief used to specify extra input stream information.
//! Pass with mxfMuxAddInputStream, mxfMuxAddInputFile, mxfMuxAddInputFileBS functions (optional).
struct mxf_stream_set_struct
{
    uint32_t stream_flags;        //!< @brief One of MXF_STREAM_FLAG_xxxx.
    uint64_t origin;              //!< @brief Number of edit units before track zero point (SMPTE-377-1 sec. 4.1)
...
struct mxf_m_settings
{
    ...
    uint32_t video_sequence_length;		      //!< deprecated, don't use

    int32_t	 reserved1[5];				          //!< Reserved for future use 

    uint32_t video_framerate;			          //!< @brief Input video frame rate (mandatory for J2K and VC3 video streams) - one of FRAMERATE_XX (mcdefs.h)

    struct mxf_product_info *product_info;	//!< @brief pointer to a mxf_product_info structure
    ...
    uint32_t dci_2k_48;					            //!< 0 or 1 - use 48fps for DCI 2K 
                                                              ...
                                                              //!< With SMPTE 428-11 now frame rates 24, 25, 30, 50, 60 fps are possible for 2K and 24, 25, 30 for 4K
    ...
    uint32_t user_duration;				          //!< @brief Preset the duration in number of video frames.
                                                              /*!< @details Mandatory for Sony XDCAM SxS and Sony XAVC SxS formats.
    ...
    uint32_t  flags;						            //!< @brief Or'ed combination of  MXF_MUX_FLAG_xxx
    ...
    uint8_t  *material_id;				          //!< @brief Pointer to a 32 byte array containing UMID for P2 or 0. 
                                                              /*!< @details If 0 UMID is generated internally.
...
extern "C" {
    ...
    //! Call to add a video or audio stream to the muxer in streaming mode
    //!
    //! @param[in]      muxer		pointer to a mxf muxer object
    //! @param[in]      input	  optional pointer to an mxf_stream_set_struct or NULL
    //! @param[in]      input		pointer to a \ref bufstream_tt object for the input data
    //! @return 0 if successful

    int32_t MC_EXPORT_API mxfMuxAddInputStream(mxf_muxer_tt *muxer, struct mxf_stream_set_struct *set, bufstream_tt *input);
    ...
    //! Call to add a video, or audio stream to the muxer in file mode
    //!
    //! @param[in]      muxer		pointer to a mxf muxer object
    //! @param[in]      input	  optional pointer to an mxf_stream_set_struct or NULL
    ...
    int32_t MC_EXPORT_API mxfMuxAddInputFile(mxf_muxer_tt *muxer, struct mxf_stream_set_struct *set, char *input);
    int32_t MC_EXPORT_API mxfMuxAddInputFileBS(mxf_muxer_tt *muxer, struct mxf_stream_set_struct *set, bufstream_tt *input);
    ...
    //! Call to wirte index into the file.
    //!
    //! @param[in]	muxer	pointer to an mxf muxer object
    //! @param[in]  idx_bs  a pointer to a bufstream to receive the index
    //!
    //! @return 0 if successful
    int32_t MC_EXPORT_API mxfMuxWriteIndexIntoFile(mxf_muxer_tt *muxer, bufstream_tt *idx_bs);
