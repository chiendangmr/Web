    ...
    namespace NetworkStreaming {
        ...
        **/                                                          
        typedef struct _tagAuthorizationField {                      
            ...
            char* pUserName;                                      /**< zero-terminated string user name */
            char* pPassword;                                      /**< zero-terminated string password */
        } AUTHORIZATIONFIELD;
        ...
        **/
        typedef struct _tagConsumerInfo {
            ...
            int64_t    tStart;                                      /**< start sample timestamp in 100NanoSec units (10e7) */
            int64_t    tStop;                                       /**< stop sample timestamp in 100NanoSec units (10e7)*/
            ...
            uint32_t   dwDecoderConfigSize;                         /**< indicates count of bytes if buffer starts
                                                                    with decoder config */
            int64_t    tPacketReceiveTime;                          /**< time in milliseconds when packet has arrived to receiver. */
            ...
            int64_t    tSegmentStart;
            int64_t    tSegmentStop;
        } CONSUMERINFO;
        ...
        **/                                                          
		...
		typedef struct _tagNetRetryInfo {

			int64_t _StartOffset; /* start offset in bytes */
            int64_t _StopOffset;  /* stop offset in bytes; -1 means till the end */
			bool     _bOffsetInTime;
			bool     _Retry;
			MCNETRESULT _ErrorType;
		} NetRetryInfo;

        typedef struct _tagDataRequestBWStat {

            int64_t duration_micro_secs;
            int64_t data_block_size_bytes;
        } DataRequestBWStat;
        ...
        **/                                                          
        typedef enum _tagNetSourceNotifyType {                       
            ...
            ESegmentStarted,										/**< Segment download started */
			ENetworkBytesReceived,									/**< Received bytes from network, argument: int32_t count of bytes */
            ERtcpPacket,                                            /**< RAW RTCP Packet Data */
			ERetry,													/**< Error happens during download @ref NetRetryInfo, should we retry? true - yes, false - no; uint32_t current offset in stream at where we've got an error */
            ENetworkBytesStarted,									/**< Triggered just before data from net socket is being read */
            ENetRequestStart,                                       /**< Reports that request which may contain multiple receives triggered (HTTP/TCP)*/
            ENetRequestEnd,                                         /**< Reports that previously fired event about request start has finished, argument: @refDataRequestBWStat amount of bytes received and duration in micro secs which was spent to download the data block */
